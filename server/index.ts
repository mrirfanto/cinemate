...
// Store movie likes by room and user
...
// Store session decisions by room and user
interface SessionDecisions {
  [roomId: string]: {
    [userId: string]: 'continue' | 'conclude';
  };
}

const movieLikes: MovieLikes = {};
const sessionDecisions: SessionDecisions = {};
...
io.on('connection', (socket: Socket) => {
  ...
  // Handle movie likes
  socket.on('movieLike', (room: string, movieId: number) => {
  ...
  socket.on(
    'sessionDecision',
    (room: string, decision: 'continue' | 'conclude') => {
      ...
      // Only proceed if there are exactly 2 users in the room
      if (roomUsers.length === 2) {
        const allDecided = roomUsers.every(
          (userId) => sessionDecisions[room][userId] !== undefined
        );

        if (allDecided) {
          // Check if both users want to conclude
          const allConclude = roomUsers.every(
            (userId) => sessionDecisions[room][userId] === 'conclude'
          );
...
function findMatchedMovies(room: string): number[] {
  if (!movieLikes[room]) return [];
  ...
  return firstUserLikes.filter((movieId) =>
    users.slice(1).every((userId) => movieLikes[room][userId].includes(movieId))
  );
}
